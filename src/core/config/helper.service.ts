...
export class HelperService
{
	...
	getFilePathAndName (file: string): { path: string; file_name: string; }
	{
		...
		const ary = file.split(DIRECTORY_SEPARATOR);
		const file_name = ary[ary.length - 1];
		let file_path = '';
	 ...
	 * @returns {any}
	 */
	async resizeImage (file: any, width: number, height: number, quality: number = 60, keepRatio: boolean = false): Promise<any>
	{
	...
	addPlayButton (image: string): any
	{
		return new Promise((resolve) =>
		{
			...
			if (mainImageDimensions.width > 2400 && mainImageDimensions.height > 2400)
			{
				playButton = './static/play_button/700x700_play-button-image.png';
			}
			else if (mainImageDimensions.width > 1500 && mainImageDimensions.height > 1500)
			{
				playButton = './static/play_button/560x560_play-button-image.png';
			}
			else if (mainImageDimensions.width > 900 && mainImageDimensions.height > 900)
			{
				playButton = './static/play_button/420x420_play-button-image.png';
			}
			else if (mainImageDimensions.width > 600 && mainImageDimensions.height > 600)
			{
				playButton = './static/play_button/280x280_play-button-image.png';
			}
			else if (mainImageDimensions.width > 300 && mainImageDimensions.height > 300)
			{
			...
			const xPostion = (mainImageDimensions.width - playButtonDimensions.width) / 2;
			const yPostion = (mainImageDimensions.height - playButtonDimensions.height) / 2;
			mergeImages([
				{ src: image, x: 0, y: 0 },
			...
			}).then((img: any) =>
			{
				...
				fs.writeFile(fileData.path + 'play_' + fileData.file_name, base64Data, 'base64', function (err: any)
				{
					console.error(err);
				});
				resolve(true);
